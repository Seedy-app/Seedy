...
const CustomInput = ({ placeholder, isPassword, isConfirmPassword, onChangeText, value }) => {
  const [isPasswordVisible, setPasswordVisible] = React.useState(true);
  const [isConfirmPasswordVisible, setConfirmPasswordVisible] = React.useState(true);
    ...
    <View>
      ...
      {isConfirmPassword && (
        <TouchableOpacity style={styles.eyeIcon} onPress={toggleConfirmPasswordVisibility}>
          <Icon name={isConfirmPasswordVisible ? 'eye-slash' : 'eye'} size={20} color="grey" />
        </TouchableOpacity>
      )}
    </View>
  );
